...
s.data_RNA <- AddMetaData(object=s.data_RNA,metadata = apply(es.max,2,function(x){names(which.max(x))}), col.name = "NT.type")
```

```{r Draw DimPlot with NT-types, fig.height=10, fig.width=10}
palette <- distinctColorPalette(length(unique(s.data_RNA$NT.type)))
DimPlot(s.data_RNA, reduction = "umap", label = TRUE, cols = palette, group.by = "NT.type") + coord_fixed()
```
