...
#s.data_RNA <- JackStraw(s.data_RNA, num.replicate = 25, dims = 50)
#s.data_RNA <- ScoreJackStraw(s.data_RNA, dims = 1:50)
```

```{r Plot Jackstraw plot}
#JackStrawPlot(s.data_RNA, dims = 1:50)
```
...
std.aim <- 0.15
std.proportion <- sapply(1:length(s.data_RNA@reductions$pca@stdev),function(x){s.data_RNA@reductions$pca@stdev[x]/s.data_RNA@reductions$pca@stdev[2]})
...
```{r RunUMAP}
s.data_RNA <- RunUMAP(s.data_RNA, dims = 1:max.lsi.dim, spread=1.5)
```

```{r Save data for chooseR interation}
qsave(s.data_RNA, file="../scRNA_data/E14_scRNA_for_chooseR.qs", nthreads = 6)
# Run choose_R_serverside_scRNA.R to iterate for optimal resolution
```
...
DimPlot(s.data_RNA, reduction = "umap", label = TRUE, cols = palette) + coord_fixed()
```
...
NT.type.markers <- gene_set_prepare2("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/onlyNT_markers311022.xlsx", "Brain")
...
mouse.cc.genes <- read_xlsx("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/CellCycle/Mouse_S_and_G2m_genes_KA.xlsx", sheet = "Sheet2")
...
pdf(height = 8, width = 8, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/scRNA_NT_type_UMAP_",run.time,".pdf",sep=""))
pushViewport(viewport(gp = gpar(fontfamily = "Helvetica")))
palette <- distinctColorPalette(length(unique(s.data_RNA$NT.type)))
DimPlot(s.data_RNA, reduction = "umap", label = FALSE, cols = palette, group.by = "NT.type") + coord_fixed() + ggtitle("")
dev.off()
```

```{r Draw DimPlot with cluster numbers, fig.height=10, fig.width=10}
run.time <- format(Sys.time(), "%d-%m-%y_%Hh%Mmin")
pdf(height = 8, width = 8, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/scRNA_UMAP_",run.time,".pdf",sep=""))
pushViewport(viewport(gp = gpar(fontfamily = "Helvetica")))
palette <- distinctColorPalette(length(levels(Idents(s.data_RNA))))
...
run.time <- format(Sys.time(), "%d-%m-%y_%Hh%Mmin")
pdf(height = 8, width = 8, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/scRNA_UMAP_brain_region_",run.time,".pdf",sep=""))
pushViewport(viewport(gp = gpar(fontfamily = "Helvetica")))
palette <- distinctColorPalette(length(unique(s.data_RNA$brain_region)))
...
saveRDS(s.data_RNA, file = paste("../scRNA_data/",sample.name,"scRNAseq.",run.date,".Rds", sep=""))
```
