...
combos <- mclapply(1:length(thrh.vector), function(x){
 message_parallel(paste0("Cluster ", x, "..."))
  if (ncol(E14data[[x]]) < 8) {
    combi(data = E14data[[x]], signalthr = thrh.vector[x], combithr = 2, max_length = 4)
  }
  else
    combi(data = E14data[[x]], signalthr = thrh.vector[x], combithr = 3, max_length = 6)
}, mc.cores = 3)
saveRDS(combos,"../../analysis/combinations.Rds")
