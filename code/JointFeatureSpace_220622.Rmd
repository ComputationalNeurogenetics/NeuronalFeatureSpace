...
pdf(file="/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/feat2TSS.pdf", width=8, height = 2.5)
plotDistToTSS(peakAnno.edb, title="Distribution of features relative to TSS")
dev.off()
```

```{r Calculating 1000 randomizations of locations and annotations, echo=FALSE}
rand.anno.vectors <- sapply(1:1000, function(r){
...
peakAnno.rand.edb <- annotatePeak(random.peaks, tssRegion=c(-3000, 3000),TxDb=edb, annoDb="org.Mm.eg.db")
  return(peakAnno.rand.edb@annoStat[,2])
})
saveRDS(rand.anno.vectors,file="rand.anno.vectors.Rds")
```

```{r Fisher.exact test of observed vs expected per annotation category, warning=FALSE}
observed.per.anno.category <- (peakAnno.edb@annoStat[,2]/100)*peakAnno.edb@peakNum
expected.per.anno.category <- (rowMeans(rand.anno.vectors)/100)*peakAnno.rand.edb@peakNum

names(observed.per.anno.category) <- peakAnno.edb@annoStat[,1]
names(expected.per.anno.category) <- peakAnno.edb@annoStat[,1]

fisher.res <- lapply(1:length(observed.per.anno.category),function(i){
  n.expected.in.categ <- expected.per.anno.category[i]
  n.expected.not.in.categ <- peakAnno.rand.edb@peakNum - n.expected.in.categ
    
  n.observed.in.categ <- observed.per.anno.category[i]
  n.observed.not.in.categ <- peakAnno.edb@peakNum - n.observed.in.categ
  
  cont.table.temp <- data.table(in.categ=c(n.expected.in.categ,n.observed.in.categ), not.in.categ=c(n.expected.not.in.categ,n.observed.not.in.categ))
  rownames(cont.table.temp) <- c("expected","observed")
  fisher.test(cont.table.temp)
})

names(fisher.res) <- names(observed.per.anno.category)
```


```{r Plotting random granges}
# Take joined feature space anno results object and replace numbers with from 1000 randomization
random.peakAnno <- peakAnno.edb
random.peakAnno@annoStat[,2] <- rowMeans(rand.anno.vectors)

plotAnnoPie(random.peakAnno)
plotAnnoBar(list(randomized=random.peakAnno,joined_feature_space=peakAnno.edb))
```
...
feat.widths <- tibble(feat.widths=ranges(clade.peaks)@width)
pdf(file="/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_length_boxplot.pdf", width = 6, height = 2)
ggplot(feat.widths,aes(x=feat.widths)) + geom_boxplot(fill="grey", outlier.shape = NA) + theme_classic() + theme(text = element_text(size=40),axis.ticks.y = element_blank(),axis.text.y = element_blank()) + xlab("Feature length") + scale_x_continuous(limits = quantile(feat.widths$feat.widths, c(0.1, 0.9)))
dev.off()
```
