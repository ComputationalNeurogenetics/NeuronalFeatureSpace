...
# Calculation atHPC cluster and returning here
...
pdf(file="../Figures/Cluster_parameters.pdf", height = 3, width = 8)
ggplot(clusterint.stats, aes(x=clus.param, group=1)) + geom_line(aes(y=dunn2, color="Dunn2")) + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_y_continuous("Dunn2", sec.axis = sec_axis(~ (. / 2.5), name = "Avg. Silhouette")) + geom_line(aes(y=avg.silhouette*2.5, color="Avg. Silhouette")) + xlab("Clade cutting height") + scale_x_continuous(breaks = seq(4, 10, by = 0.5))  + theme(legend.title=element_blank(), text = element_text(size=16))
dev.off()
```
...
```{r Getting barcode to cluster mapping for each region}

# Read NT types information
nt.types <- readxl::read_xlsx("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Cell types master table 280622 joint feature space.xlsx", sheet="cell types labels") %>% dplyr::select(`Cluster id`, Neurotransmitter)

# Getting barcode to cluster mapping for each region
...
#
#barcode.to.cluster.mapping.mb <- tibble(barcode=paste("_e14vmb_",names(Idents(s.data.mb)),sep=""), cluster=paste("mb.",Idents(s.data.mb),sep=""))
#
# barcode.to.cluster.mapping.di <- tibble(barcode=paste("_e14di_",names(Idents(s.data.di)),sep=""), cluster=paste("di.",Idents(s.data.di),sep=""))
#
# barcode.to.cluster.mapping.e12r1 <- tibble(barcode=paste("_e12r1_",names(Idents(s.data.e12r1)),sep=""), cluster=paste("e12r1.",Idents(s.data.e12r1),sep=""))
#
# barcode.to.cluster.mapping.e14r1 <- tibble(barcode=paste("_e14r1_",names(Idents(s.data.e14r1)),sep=""), cluster=paste("e14r1.",Idents(s.data.e14r1),sep=""))

#combined.mappings <- bind_rows(barcode.to.cluster.mapping.mb,barcode.to.cluster.mapping.di,barcode.to.cluster.mapping.e12r1,barcode.to.cluster.mapping.e14r1)

#saveRDS(combined.mappings, file="../analysis//combined.barcode.to.cluster.mappings.Rds")

combined.mappings <- readRDS("../analysis/combined.barcode.to.cluster.mappings.Rds")

# Please note that combined mappings contains only cells which have went through additional QC after peak detection.

nt.typed.barcodes <- left_join(combined.mappings, nt.types, by=c("cluster"="Cluster id"))

nt.typed.clades.with.barcodes <- left_join(lsi_cells, nt.typed.barcodes)

# Change all cells not present in cluster level analysis but still here to have status Unknown for their NT type

nt.typed.clades.with.barcodes[is.na(nt.typed.clades.with.barcodes$Neurotransmitter),"Neurotransmitter"] <- "Unknown"

nt.type.counts <- table(nt.typed.clades.with.barcodes %>% dplyr::select(Neurotransmitter, cells_tree_cut))

# Change all cells not anymore present in cell cluster stage to have NT type Unknown
nt.type.prop <- apply(nt.type.counts,2,function(x){
  if(sum(x)>0){
    x/sum(x)
    } else {x}
  })
...
```{r Draw circos plot with NT types stacked barplot, fig.height=32, fig.width=32}
hcd.upper.circ  <- hcd.upper
par(mar = rep(4,4))
circos.initialize("leaf.labels", xlim = c(0, ncol(nt.type.counts)))

circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.text(1:ncol(nt.type.counts)-0.5, rep(0, ncol(nt.type.counts)), labels(hcd.upper.circ), col = labels_colors(hcd.upper.circ),
               facing = "clockwise", niceFacing = TRUE, adj = c(0, 0.5), cex=1.2)
}, bg.border = NA, track.height = 0.15)
max_height = attr(hcd.upper.circ, "height")

circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(t(nt.type.prop),1:ncol(nt.type.counts)-0.5, col=pal_jco()(8))
}, track.height =.15, bg.border = NA)
...
lgd_barplot_nt.type = 
  Legend(at = rownames(nt.type.prop), type = "points",labels_gp = gpar(col = "black", cex = 2), legend_gp = gpar(col=pal_jco()(8), lwd = 2), title_position = "topleft", title_gp = gpar(cex = 2), title = "NT type", size = unit(4, "mm")
    )

lgd_list_vertical = packLegend(lgd_barplot_nt.type)

draw(lgd_list_vertical, x = unit(2, "cm"), y = unit(2, "cm"), just = c("left", "bottom"))
```

```{r Draw circos with combined NT and brain region stacked barplot tracks, fig.height=28, fig.width=28}
hcd.upper.circ  <- hcd.upper
par(mar = rep(4,4))
circos.par(track.margin=c(0.0001, 0.0001), cell.padding=c(0.01,0.01), gap.after=.5)
circos.initialize("leaf.labels", xlim = c(0, ncol(nt.type.counts)))
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(t(sample.clade.prop),1:ncol(sample.clade)-0.5, col=pal_jco()(4))
}, track.height =.12, bg.border = NA)

circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(t(nt.type.prop),1:ncol(nt.type.counts)-0.5, col=pal_lancet()(8))
}, track.height =.12, bg.border = NA)

circos.track(ylim = c(0, max_height), panel.fun = function(x, y) {
   circos.dendrogram(hcd.upper.circ, max_height = max_height)
}, track.height =.41, bg.border = NA)

lgd_barplot_nt.type = 
  Legend(at = rownames(nt.type.prop), type = "points",labels_gp = gpar(col = "black", cex = 4), legend_gp = gpar(col=pal_lancet()(8), lwd = 3), title_position = "topleft", title_gp = gpar(cex = 3), title = "NT type", size = unit(8, "mm"))

lgd_barplot_region = 
  Legend(at = rownames(sample.clade.prop), type = "points",labels_gp = gpar(col = "black", cex = 4), legend_gp = gpar(col=pal_jco()(4), lwd = 3), title_position = "topleft", title_gp = gpar(cex = 3), title = "Brain region", size = unit(8, "mm"))

lgd_list_vertical <- packLegend(lgd_barplot_region, lgd_barplot_nt.type)    
...
```{r}
peakAnno.edb <- annotatePeak(clade.peaks, tssRegion=c(-3000, 3000),
...
```{r Comparison to all ENCODE mouse cCREs}
# ENCODE.cCREs <- readGeneric("../../mm10/41586_2020_2493_MOESM13_ESM.txt", meta.cols = list(rDHS_accession = 4, cCRE_accession = 5, group = 6), remove.unusual = TRUE, header=TRUE)
# DefaultAssay(s.data.e14) <- "peaks"
# features.gr <- StringToGRanges(rownames(s.data.e14))
```
...
makeVennDiagram(Peaks=list(clade.peaks,unique(Combined.E14.brain.cCREs)), NameOfPeaks = c("scATAC features","ENCODE cCREs"), minoverlap = 1)
dev.off()
```
...
ggplot(tibble(overlap=percentOverlap),aes(x=overlap)) + geom_histogram(colour="black", fill=pal_jco()(6)[6]) + theme_minimal() + xlab("Overlap proportion") + ggtitle("Hist. of E14DI separate feature space feature overlaps\nwith the joint one") + theme(text = element_text(size=14))
...
ggplot(tibble(overlap=percentOverlap),aes(x=overlap)) + geom_histogram(colour="black", fill=pal_jco()(6)[6]) + theme_minimal() + xlab("Overlap proportion") + ggtitle("Hist. of joint feature space feature overlaps\nwith the E14DI separate one") + theme(text = element_text(size=14))
```

```{r VENN diagram Joint vs E14DI vs E14vMB, fig.height=6, fig.width=6}
pdf(file="Joint_E14DI_E14vMB.pdf", width = 6, height = 6)
...
pdf(file="Joint_E12R1_E12R1.pdf", width = 6, height = 6)
...
pdf(file="VENN_all.pdf", width = 15, height = 15)
