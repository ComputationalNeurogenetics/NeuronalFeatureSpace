...
```{r}
# NT.mean.scores <- sapply(levels(Idents(s.data)), function(id){i <- which(as.character(Idents(s.data))==id); rowMeans(es.max[,i])})
# create_dt(NT.mean.scores)
```

```{r Draw NT type UMAP}
DimPlot(s.data, group.by = 'NT.type', pt.size = 0.1) + ggtitle("Neurotransmitter type") + coord_fixed()
```
