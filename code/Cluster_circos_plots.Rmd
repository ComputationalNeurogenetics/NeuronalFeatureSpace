...
closest.gene.1 <- ClosestFeature(object=s.data, regions=feat.1)$gene_name
closest.gene.2 <- ClosestFeature(object=s.data, regions=feat.2)$gene_name
...
s.data.e14 <- qread("../scATAC_data/E14_DownstreamReady.300922.RNA.pos.idents.qs", nthreads = 6)
```
...
da_bp_di_gaba <- lapply(da_features[c(54, 75, 116)],function(da.1){
  lapply(da.1[c(54, 75, 116)], function(da.2){
    if (is_tibble(da.2)){
...
rownames(dalength.matrix) <- as.character(c(54, 75, 116))
colnames(dalength.matrix) <- as.character(c(54, 75, 116))
```
...
dat.1 <- prepare_comp_circos_data(da_features = da_features, id.1=id.1,id.2=id.2, avg_log2FC.limit = 0.75,  s.data=s.data.e14)
...
dat.1 <- prepare_comp_circos_data(da_features = da_features, id.1=id.1,id.2=id.2, s.data=s.data.e14)
