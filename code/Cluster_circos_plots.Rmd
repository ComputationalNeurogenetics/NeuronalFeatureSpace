...
s.data.e14 <- qread("../scATAC_data/E14_DownstreamReady.280823.RNA.pos.idents.qs", nthreads = 6)
scRNA.e14.data <- readRDS("../scRNA_data/E14scRNAseq.240823.Rds")
```
...
```{r Calculate number of clusters with >.5 mojority in regions and NT-types}
# Repeat for b in 1:3
lapply(1:8,function(i){sum(nt.type.prop[i,]>0.55 & brain.region.prop[b,]>0.55)})
# and
lapply(1:8,function(i){sum(nt.type.prop[i,]>0.5)})

#And same for scRNA object
lapply(1:8,function(i){sum(nt.type.prop.scRNA[i,]>0.55 & brain.region.prop.scRNA[b,]>0.55)})
# and
...
```{r Add 50% cluster type calls to CombiROC results}
combiroc.tb <- readxl::read_xlsx("../analysis/CombiROC_results.xlsx")[,c(1,4)]

supp.table.4 <- left_join(combiroc.tb, nt.types.per.cluster.50) %>%  left_join(brain.regions.per.cluster.50)

create_dt(supp.table.4)
```
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(value=t(embryo.type.prop)[as.numeric(labels(hcd_e14)),],pos=1:nrow(embryo.type.counts)-0.5, col=pal_d3("category20")(4))
}, track.height =.15, bg.border = NA)
...
lgd_barplot = packLegend(
   Legend(at = colnames(t(embryo.type.prop)[as.numeric(labels(hcd_e14)),]), type = "lines",labels_gp = gpar(col = "black", cex = 2.5), legend_gp = gpar(col=pal_d3()(4), lwd = 8), title_position = "topleft", title_gp = gpar(cex = 2), title = "Embryo", size = unit(4, "mm")),direction="horizontal")
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(value=t(genotype.type.prop)[as.numeric(labels(hcd_e14)),],pos=1:nrow(genotype.type.counts)-0.5, col=pal_jco()(2))
}, track.height =.15, bg.border = NA)
...
lgd_barplot = packLegend(
   Legend(at = colnames(t(genotype.type.prop)[as.numeric(labels(hcd_e14)),]), type = "lines",labels_gp = gpar(col = "black", cex = 2.5), legend_gp = gpar(col=pal_jco()(2), lwd = 8), title_position = "topleft", title_gp = gpar(cex = 2), title = "Genotype", size = unit(4, "mm")),direction="horizontal")
...
lgd_barplot = packLegend(
   Legend(at = colnames(t(nt.type.prop)[as.numeric(labels(hcd_e14)),]), type = "lines",labels_gp = gpar(col = "black", cex = 2.5), legend_gp = gpar(col=pal_jco()(9)[-8], lwd = 8), title_position = "topleft", title_gp = gpar(cex = 2), title = "Embryo", size = unit(4, "mm")),direction="horizontal")
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(value=t(brain.region.prop)[as.numeric(labels(hcd_e14)),],pos=1:nrow(brain.region.counts)-0.5, col=pal_d3("category20")(3))
}, track.height =.15, bg.border = NA)
...
lgd_barplot = packLegend(
   Legend(at = colnames(t(brain.region.prop)[as.numeric(labels(hcd_e14)),]), type = "lines",labels_gp = gpar(col = "black", cex = 2.5), legend_gp = gpar(col=pal_d3("category20")(3), lwd = 8), title_position = "topleft", title_gp = gpar(cex = 2), title = "Embryo", size = unit(4, "mm")),direction="horizontal")
...
Dlx.GABA.e14.data <- subset(s.data.e14, idents=c(109,83,54,43))
...
row_ha <- rowAnnotation(scATAC_cluster = Idents(Dlx.GABA.e14.data), col = list(scATAC_cluster = c("109" = "red", "83" = "green", "54" = "steelblue", "43" = "orange")),annotation_name_gp= gpar(fontsize = 20), annotation_legend_param = list(title = "Cluster", labels_gp=gpar(fontsize=16), title_gp = gpar(fontsize=16)))
...
# DI GABA only
#c(which(nt.type.prop["GABAergic neurons",]>0.75 & brain.region.prop["di",]>0.75),c(101,24,15))

#nt.type.prop["GABAergic neurons",]>0.75  &

# GABA Only
# c(c(which(nt.type.prop["GABAergic neurons",]>0.75),c(101,24,15)))

GABA.prog.e14.data <- subset(s.data.e14, 
idents= intersect(c(c(which(nt.type.prop["GABAergic neurons",]>0.75 & brain.region.prop["di",]>0.5),c(15))), reliable.clusters)
)
```
...
write.table(VIA.out, file="../analysis/VIA.out.GABA.DI.csv", sep = ",")
```
