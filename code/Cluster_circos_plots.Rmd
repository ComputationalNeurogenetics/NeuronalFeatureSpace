...
embryo[replicates %in% c("di_0","di_1","mb_0","mb_1","r1_1")] <- "embryo1"
embryo[replicates %in% c("di_2","mb_2","r1_2","r1_0")] <- "embryo2"
s.data.e14 <- AddMetaData(s.data.e14, metadata = embryo, col.name = "embryo")

palette <- ggsci::pal_igv()(length(unique(s.data.e14$embryo)))
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
   circos.barplot(value=t(embryo.type.prop)[as.numeric(labels(hcd_e14)),],pos=1:nrow(embryo.type.counts)-0.5, col=pal_d3("category20")(2))
}, track.height =.15, bg.border = NA)
...
lgd_barplot = packLegend(
   Legend(at = colnames(t(embryo.type.prop)[as.numeric(labels(hcd_e14)),]), type = "lines",labels_gp = gpar(col = "black", cex = 2.5), legend_gp = gpar(col=pal_d3()(2), lwd = 8), title_position = "topleft", title_gp = gpar(cex = 2), title = "Embryo", size = unit(4, "mm")),direction="horizontal")
