...
hcd_e14 <- hcd_e14 %>% color_branches(k=5)
#prunned_dends <- prune_cutree_to_dendlist(hcd_e14,2)
...
```{r global rel counts}
global.res <- lapply(2:45, function(k){
...
```{r brain rel counts}
brain.res <- lapply(2:45, function(k){
  prunned_dends <- prune_cutree_to_dendlist(hcd_e14,k)
  res.tb.list <- lapply(1:length(prunned_dends),function(d){
...
pl.1.region <- lapply(1:length(brain.res),function(r){
  if (r==1){
      ggplot(brain.res[[r]], aes(fill=attribute, y=rel.enr, x=prunned_tree)) + geom_bar(position="stack", stat="identity") + scale_fill_brewer(palette = "Set3") + theme(axis.text.x = element_blank(), legend.text = element_text(colour="black", size = 6, face = "bold"),legend.key.size = unit(.25, 'cm')) + xlab("Branches") 
  } else {
      ggplot(brain.res[[r]], aes(fill=attribute, y=rel.enr, x=prunned_tree)) + geom_bar(position="stack", stat="identity") + scale_fill_brewer(palette = "Set3") + theme(axis.text.x = element_blank(), legend.position="none") + xlab("Branches") 
  }
})
```
...
pl.1.nt.type <- lapply(1:length(nt.res),function(r){
  if (r==1){
      ggplot(nt.res[[r]], aes(fill=attribute, y=rel.enr, x=prunned_tree)) + geom_bar(position="stack", stat="identity") + scale_fill_brewer(palette = "Set3") + theme(axis.text.x = element_blank(), legend.text = element_text(colour="black", size = 6, face = "bold"),legend.key.size = unit(.25, 'cm')) + xlab("Branches") 
  } else {
      ggplot(nt.res[[r]], aes(fill=attribute, y=rel.enr, x=prunned_tree)) + geom_bar(position="stack", stat="identity") + scale_fill_brewer(palette = "Set3") + theme(axis.text.x = element_blank(), legend.position="none") + xlab("Branches") 
  }
})
```


```{r Plot into multipage pdf}
pdf(file = "../analysis/tree.branching.region.up.to.44.pdf", width = 8, height = 15, onefile = TRUE)
patchwork::wrap_plots(pl.1.region[1:15], ncol = 1)
patchwork::wrap_plots(pl.1.region[16:31], ncol = 1)
patchwork::wrap_plots(pl.1.region[32:44], ncol = 1)
dev.off()
...
patchwork::wrap_plots(pl.1.region[1:10], ncol = 1)
dev.off()
```
...
patchwork::wrap_plots(pl.1.nt.type[1:10], ncol = 1)
dev.off()
```
