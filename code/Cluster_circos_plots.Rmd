...
scRNA.e14.data <- readRDS("scRNA_data/E14scRNAseq.071222.Rds")
```
...
```{r Calculate number of clusters with >.75 mojority in regions and NT-types}
# Repeat for b in 1:3
lapply(1:8,function(i){sum(nt.type.prop[i,]>0.75 & brain.region.prop[b,]>0.75)})
# and
lapply(1:8,function(i){sum(nt.type.prop[i,]>0.75)})
#And same for scRNA object
lapply(1:8,function(i){sum(nt.type.prop.scRNA[i,]>0.75 & brain.region.prop.scRNA[b,]>0.75)})
# and
lapply(1:8,function(i){sum(nt.type.prop.scRNA[i,]>0.75)})
```
...
```{r DE genes between Di DLX GABA pairs}
# Pair 18 vs 60
dlx.cluster.markers.pair.1 <- rownames_to_column(FindMarkers(Dlx.GABA.e14.data, assay = "RNA_name", ident.1 = 18, ident.2 = 60), var = "gene_symbol") %>% as_tibble()

closest.genes.pair.1 <- prepare_comp_circos_data(da_features = da_features, id.1=18,id.2=60, s.data=s.data.e14, avg_log2FC.limit = 0.7)

intersect(closest.genes.pair.1 %>% pull(label), dlx.cluster.markers.pair.1 %>% dplyr::filter((avg_log2FC > 0.5 | avg_log2FC < -0.5) & p_val_adj <0.05) %>% pull(gene_symbol))


# Pair 18 vs 60
dlx.cluster.markers.pair.2 <- rownames_to_column(FindMarkers(Dlx.GABA.e14.data, assay = "RNA_name", ident.1 = 122, ident.2 = 19), var = "gene_symbol") %>% as_tibble()

closest.genes.pair.2 <- prepare_comp_circos_data(da_features = da_features, id.1=122,id.2=19, s.data=s.data.e14, avg_log2FC.limit = 0.7)

intersect(closest.genes.pair.2 %>% pull(label), dlx.cluster.markers.pair.2 %>% dplyr::filter((avg_log2FC > 0.5 | avg_log2FC < -0.5) & p_val_adj <0.05) %>% pull(gene_symbol))
