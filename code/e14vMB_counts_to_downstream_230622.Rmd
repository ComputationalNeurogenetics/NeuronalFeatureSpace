---
title: "R Notebook of E14VMB downstream analysis from binarized peak level data with "
...
```{r System dependiencies, include=FALSE}
if (Sys.info()['sysname']=="Darwin") {
    ...
    cores=7
    data_path <- "/VarastoPool/University_research_data/"
    supp_data_path <- "/VarastoPool/University_research_data/mm10/"
}
```
...
s.data <- readRDS(paste("../scATAC_data/",sample.name,".merged.peaks.220622.Rds",sep=""))
s.data_RNA <- readRDS("../scRNA_data/E14VMBscRNAseq.031121.Rds")
...
Plotting read depth correlation plot, in Seurat pipeline usually first PC1 is omitted because of this association, however, now data comes in binary format and effect of this is not that clear. Currently PC1 is included.

### scATAC UMAP clusters

```{r UMAP and cluster detection, fig.width=14, fig.height=14}
...
Run UMAP, and find clusters, plot

### scATAC clades projection into UMAP clusters

```{r Testing how clades from phase 1 situate in UMAP projection}
# Curretnly not implemented as clade information is no longer inherit from clade peak phase as it is done separately and jointly
...
#s.data_RNA@meta.data$CellType<-as.factor(cluster_id_name$ClusterName[match(s.data_RNA@meta.data$seurat_cluster,cluster_id_name$cluster)])
...
scATAC seq gene activity quantification done previously with assumption of gene activity signal being (normalized) count of reads up to 2kbp upstream of startTx
Activity data stored in s.data 'Activity' assay ATAC data has already been processed (LSI SVD and UMAP) under 'peaks' assay of s.data object

Note "label transfer" obviously needs labels to transfer. In this case clusters in the scRNA seq needs
to be named. Using now just alphabetical version of seurat cluster numbering available in s.data_rna object
...
```{r Select cells with accepted prediction score and plot results of label transfer, fig.height=14, fig.width=14}
...
p2 <- DimPlot(s.data_RNA, group.by = "CellType", label = TRUE, repel = TRUE, label.size=7) + NoLegend()
...
```{r Regenerate UMAP and cluster detection after scRNA mapping score subsetting, fig.width=14, fig.height=14}
DefaultAssay(s.data) <- "peaks"
s.data <- RunUMAP(object = s.data, reduction = 'lsi', dims = 2:30)
s.data <- FindNeighbors(object = s.data, reduction = 'lsi', dims = 2:30)
s.data <- FindClusters(object = s.data, verbose = FALSE, algorithm = 4)
