...
s.data <- FindNeighbors(object = s.data, reduction = 'lsi', dims = 2:max.lsi.dim.re, graph.name = c("S.lsi.peaks","N.lsi.peaks"))
qsave(s.data, file="../scATAC_data/s.data.merged.for.chooseR.qs", nthreads = 6)
```
...
scores <- readRDS("../chooseR/results_leiden/scores.Rds")
...
DimPlot(object = s.data, label = TRUE, pt.size=.6, label.size = 2) + NoLegend()
```
...
f.plot.tmp <- FeaturePlot(
  object = s.data,
  features = c(neuronal.markers.ids),
  pt.size = 0.1,
  max.cutoff = 'q95',
  combine = F
)
...
patchwork::wrap_plots(f.plots.2, ncol = 6)
```
...
```{r}
# Calculate with DA_chromatin.R in HPC cluster and read results
da_features_13 <- qread("../scATAC_data/da_features_13.qs", nthreads = 6)

da_bp_13 <- lapply(da_features_13,function(da.1){
...
dalength.matrix.13 <- sapply(da_bp_13, unlist)/1000
rownames(dalength.matrix.13) <- as.character(seq(1,nrow(dalength.matrix.13),by=1))
colnames(dalength.matrix.13) <- as.character(seq(1,ncol(dalength.matrix.13),by=1))
```

```{r Pairwise chromatin DA count}
da_count_13 <- lapply(da_features_13,function(da.1){
  lapply(da.1, function(da.2){
    if (is_tibble(da.2)){
      filt.da <- da.2 %>% dplyr::filter((avg_log2FC > 0.75 | avg_log2FC < -0.75) & p_val_adj < 0.05) %>% pull(feature)
      if (length(filt.da)>0){
...
dacount.matrix.13 <- sapply(da_count_13, unlist)
rownames(dacount.matrix.13) <- as.character(seq(1,nrow(dacount.matrix.13),by=1))
colnames(dacount.matrix.13) <- as.character(seq(1,ncol(dacount.matrix.13),by=1))
```

```{r Draw clustered heatmap of pairwise DA results, fig.width=18, fig.height=18}
col_fun = colorRamp2(c(0, median(dalength.matrix.13),max(dalength.matrix.13)), c("white","yellow", "red"))
Heatmap(dalength.matrix.13, col = col_fun, clustering_distance_rows = "euclidean", clustering_distance_columns = "euclidean", clustering_method_rows = "ward.D2", clustering_method_columns = "ward.D2", heatmap_legend_param = list(legend_height = unit(6, "cm"),grid_width = unit(1, "cm"),labels_gp = gpar(col = "black", font = 20)))
```

```{r Histogram of DA feature chromatin kbp level}
hist(dalength.matrix.13[upper.tri(dalength.matrix.13)], main = "Histogram of DA feature lengths between all clusters", xlab = "kbp")
```
```{r Histogram of DA feature count}
hist(dacount.matrix.13[upper.tri(dacount.matrix.13)], main = "Histogram of DA feature counts between all clusters", xlab = "Feature count")
```
# Adding Motif information into the object

```{r Adding Motif information to the object, echo=FALSE}
