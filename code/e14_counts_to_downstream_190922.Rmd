...
#qsave(s.data, file="../scATAC_data/s.data.merged.for.chooseR_q0.qs", nthreads = 6)
```
...
scores <- readRDS("../chooseR/results_leiden_100_q0/scores.Rds")

# Add cluster specific silhouette scores as metadata to s.data? Is there cell specific? Check publication
s.data <- AddMetaData(object = s.data, metadata = scores$avg_sil[match(as.numeric(s.data$S.lsi.peaks_res.12), scores$cluster)], col.name = "silh.score")
```
...
qsave(s.data, file="../scATAC_data/s.data.res12.qs", nthreads = 6)
```
...
Heatmap(dalength.matrix, col = col_fun, heatmap_legend_param = list(legend_height = unit(6, "cm"),grid_width = unit(1, "cm"),labels_gp = gpar(col = "black", font = 20)), cluster_rows=TRUE, cluster_columns = TRUE)
```
