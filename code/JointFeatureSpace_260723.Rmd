---
title: R Notebook of studies into defining joint neuronal scATAC feature space for mouse CNS tissues E14
...
system.time(neuronal.merged <- merge(x = e14di.merged, y = c(e14mb.merged, e14vr1.merged), add.cell.ids = c("_e14di", "_e14vmb","_e14vr1")))
```
...
qsave(neuronal.merged, paste("../scATAC_data/neuronal.merged.clade.phase.",run.date,".qs",sep=""), nthreads = 6)
```
...
embryo.batch[replicate %in% c("_e14mb_1", "_e14di_1","_e14r1_1","_e14mb_2", "_e14di_2","_e14r1_2")] <- "6.10.20"
...
qsave(neuronal.merged.filt, paste("../scATAC_data/neuronal.merged.clade.phase.filt.",run.date,".qs",sep=""), nthreads = 6)
```
...
```{r}
qsave(hclust_cells,paste("../scATAC_data/hclust.",run.date,".qs",sep=""))
qsave(cell.dist,paste("../scATAC_data/cell.dist.",run.date,".qs",sep=""))
```
...
km.stats.list <- qread("../analysis/cluster.stats.list.260723.qs", nthreads = 6)
...
clusterint.stats <- tibble(clus.param=seq(4,10,by=.5), dunn2=dunn2, avg.silhouette=avg.silhouette)

pdf(height = 3, width = 8, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Clade_level_cluster_parameters_",run.date,".pdf",sep=""))
...
pdf(file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Clade_circos_batch.",run.date,".pdf",sep=""), width = 24, height = 24)
...
clade.peaks <- CallPeaks(neuronal.merged.filt, macs2.path = "/Users/kilpinen/opt/anaconda3/envs/r411_291021/bin/macs2", effective.genome.size = 1.87e9, group.by = "clade", combine.peaks = TRUE, name = paste(sample.name,"_per_clade_peaks_merged", sep=""))
```
...
clade.peaks <- readRDS(paste("../analysis/joint.clade.peaks.",run.date,".Rds",sep=""))
```
...
There are 470 578 features in jointly defined feature space

```{r Boxplot of feature lengths}
pdf(height = 8, width = 12, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_length_boxplot.",run.date,".pdf",sep=""))
feature.lengths <- ranges(clade.peaks)@width
outlier.thr <- quantile(x = feature.lengths, .75)+IQR(feature.lengths)*1.5
boxplot(feature.lengths[feature.lengths<outlier.thr], horizontal=TRUE)
dev.off()
```


```{r Stats of features, fig.width=12}
pdf(height = 8, width = 12, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_length_histogram.",run.date,".pdf",sep=""))
pushViewport(viewport(gp = gpar(fontfamily = "Helvetica")))
...
pdf(height = 12, width = 12, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_density_circos.",run.date,".pdf",sep=""))
pushViewport(viewport(gp = gpar(fontfamily = "Helvetica")))
...
```{r Calculcate average.feature.loc.gene for clade.peaks}
...
pdf(width = 9, height = 3, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_distribution_to_TSS.",run.date,".pdf",sep=""))
...
```{r Draw stacked barplot version of the pie charts, fig.height=6, fig.width=8}
peak.anno.act <- tibble(Feature=peakAnno.edb@annoStat$Feature, Frequency=peakAnno.edb@annoStat$Frequency, Case="scATAC features")

peak.anno.rand <- tibble(Feature=peakAnno.rand.edb@annoStat$Feature, Frequency=peakAnno.rand.edb@annoStat$Frequency, Case="Random")

combined.anno <- rbind(peak.anno.act,peak.anno.rand)

pdf(width = 8, height = 6, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_stackedbarplot.",run.date,".pdf",sep=""))
ggplot(combined.anno, aes(fill=Case, y=Frequency, x=Feature)) + 
    geom_bar(position="dodge", stat="identity") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=20), axis.text.y = element_text(size=20)) + xlab("")
dev.off()
```

```{r Statistical calculation for Supp Table 1}

total.feature.count <- length(clade.peaks)

fishers <- lapply(1:11,function(i){
  clade.tmp <- ((peakAnno.edb@annoStat[i,]$Frequency)/100)*total.feature.count
  random.tmp <- ((peakAnno.rand.edb@annoStat[i,]$Frequency)/100)*total.feature.count
  return(fisher.test(as.table(matrix(c(random.tmp,total.feature.count-random.tmp,clade.tmp, total.feature.count-clade.tmp),nrow=2,ncol=2))))
})
names(fishers) <- peakAnno.edb@annoStat[,1]
fishers
```


```{r Intersect of feature space to ENCODE}
s.data <- qread("../scATAC_data/E14_DownstreamReady.080923.RNA.pos.idents.qs", nthreads = 6)

feature.space <- StringToGRanges(rownames(s.data))
overlapping <- subsetByOverlaps(feature.space, Combined.brain.E14.cCREs, minoverlap = 1L)

non.overlapping <- subsetByOverlaps(feature.space, Combined.brain.E14.cCREs, minoverlap = 1L, invert = TRUE)

```

```{r Draw stacked barplot of feature space to ENCODE comparison, fig.height=6, fig.width=8}
peakAnno.over <- annotatePeak(overlapping, tssRegion=c(-3000, 3000),TxDb=edb, annoDb="org.Mm.eg.db")

peakAnno.non.over <- annotatePeak(non.overlapping, tssRegion=c(-3000, 3000),TxDb=edb, annoDb="org.Mm.eg.db")

peakAnno.over.tb <- tibble(Feature=peakAnno.over@annoStat$Feature, Frequency=peakAnno.over@annoStat$Frequency, Case="Overlapping ENCODE")

peakAnno.non.over.tb <- tibble(Feature=peakAnno.non.over@annoStat$Feature, Frequency=peakAnno.non.over@annoStat$Frequency, Case="Non-overlapping ENCODE")

combined.anno <- rbind(peakAnno.over.tb,peakAnno.non.over.tb)

pdf(width = 8, height = 6, file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/Feature_space_ENCODE_stackedbarplot.",run.date,".pdf",sep=""))
ggplot(combined.anno, aes(fill=Case, y=Frequency, x=Feature)) + 
    geom_bar(position="stack", stat="identity") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=20), axis.text.y = element_text(size=20)) + xlab("")
dev.off()
```
...
pdf(file=paste("/Users/kilpinen/OneDrive - University of Helsinki/Neuronal Feature Space/Figures/plots for figures/VENN_E14_BRAIN_ENCODE_vs_Feature_space.",run.date,".pdf",sep=""), width = 15, height = 15)
makeVennDiagram(Peaks=list(feature.space,unique(Combined.brain.E14.cCREs)), NameOfPeaks = c("scATAC features","ENCODE cCREs"), minoverlap = 1)
dev.off()
```
